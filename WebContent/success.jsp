...
<body>
	<h2>Success page</h2>
	<div>
		<p>Here again, we see the token injected into src and href attributes:</p>
		<p>
			<a href="jsp2jsp.jsp?parm1=sampleparm1&parm2=sampleparm2" >Click here to be sent directly to another JSP.</a><br/>
			<a href="#webXml">Click here to view web.xml configuration.</a>
		...
		<p>Frank is here only as a demonstration of autowiring dependencies and using JSTL.</p>
		<p> <!--  #scriptletsSuck #useELerryday #ELsavedMyMarriage -->
		...
		<hr/>
		<div>
			<p>Ajax calls are essentially protected "for free":</p>
		</div>
		<div>
			<input type="button" id="ajaxButton" value="sendAJAX" />
		</div>
		<div>
			<p>We do not have to explicitly code for the token:</p>
			<p>
		...
		<div>
			<p>This select box is an example of when simply including the call to JavaScriptServlet is not enough, so we use the tag library.  The option looks like this:</p>
			<p>
				<code class="prettyprint" style="border:none!important;">
					${optionSource}
				</code>
			</p>
			<p>And is generated by JSP code that looks like this:</p>
			<p>
				<pre class="prettyprint" style="border:none!important;">${jspCode}</pre>
			</p>
		</div>
		<br/>
		<div>
			<p>This function is attached to the onchange event:</p>
			<p>
				<pre class="prettyprint" style="border:none!important;">
	...
	<div>
		<a name="webXml" ></a>
		<pre class="prettyprint" style="border:none!important;">${webXml}</pre>
	</div>
	<script>
